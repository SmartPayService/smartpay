...
async function generateReceiptPDF(data){
  const container=document.createElement('div');container.style.padding='16px';container.style.fontFamily='Arial, sans-serif';
  container.innerHTML=`
  <div style="border:1px solid #cdd6e1;border-radius:12px;padding:20px;">
   ...
   <table style="width:100%;font-size:14px;border-collapse:collapse">
    ${Object.entries(data.lines).map(([k,v])=>`<tr><td style="padding:6px 4px;color:#374151;width:35%">${k}</td><td style="padding:6px 4px;color:#111827;font-weight:600;">${v}</td></tr>`).join('')}
   </table><hr>
   <div style="display:flex;justify-content:space-between;font-weight:700;"><span>Total</span><span>₹${data.total}</span></div>
   <div style="margin-top:12px;font-size:12px;color:#6b7280;">This is a system generated receipt for demo/testing.</div>
  </div>`;
...
function makeTxnId(){return 'SP'+Math.random().toString(36).slice(2,8).toUpperCase()+Date.now().toString().slice(-4);}
